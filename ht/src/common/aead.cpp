                        ...
                        std::string& key_out)
{
    ...
    key_out.assign(reinterpret_cast<const char*>(mac), 32);
    return true;
}
                       ...
                       const std::string& aad)
{
    ...
    do {
        ...
        if (1 != EVP_DecryptInit_ex(ctx, nullptr, nullptr,
                                    ...
                                    reinterpret_cast<const unsigned char*>(nonce12.data()))) break;
        ...
        const unsigned char* tag =
            reinterpret_cast<const unsigned char*>(ciphertext.data()) + ct_len;
        if (1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, 16,
        ...
        if (ct_len > 0) {
            if (1 != EVP_DecryptUpdate(ctx,
                                       reinterpret_cast<unsigned char*>(plaintext.data()), &outl,
    ...
    do {
        ...
        if (!plaintext.empty()) {
            if (1 != EVP_EncryptUpdate(ctx,
                                       reinterpret_cast<unsigned char*>(ciphertext.data()), &outl,
