...
bool parse_request_line(const std::string& line, ht::HttpRequest& r) {
    ...
    if (!(iss >> r.method)) return false;
    std::string uri;
    if (!(iss >> uri)) return false;
    if (!(iss >> r.httpver)) return false;
    ...
    if (qpos == std::string::npos) {
        ...
        r.path  = uri.substr(0, qpos);
        r.query = uri.substr(qpos + 1);
    }
    return true;
}
...
std::unordered_map<std::string, std::string> parse_query(const std::string& q) {
    ...
    std::size_t p = 0;
    while (p < q.size()) {
        std::size_t eq  = q.find('=', p);
        std::size_t amp = q.find('&', p);
        if (eq == std::string::npos) break;
        std::string k = url_decode(q.substr(p, eq - p));
        std::string v = url_decode(q.substr(eq + 1,
                              (amp == std::string::npos ? q.size() : amp) - (eq + 1)));
        m[k] = v;
